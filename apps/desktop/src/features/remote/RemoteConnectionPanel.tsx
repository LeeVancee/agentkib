...
import { Monitor, RefreshCw, ShieldCheck, Wifi } from "@octanejs/lucide";
        ...
        >
          <label className="grid gap-1.5 text-sm">
            ...
            <Input
              ...
              value={address}
              onChange={(event: any) => setAddress((event.target as HTMLInputElement).value)}
              disabled={busy}
              autoComplete="off"
            />
          </label>
          <label className="grid gap-1.5 text-sm">
            ...
            <Input
              value={code}
              onChange={(event: any) =>
                setCode((event.target as HTMLInputElement).value.replace(/\D/g, "").slice(0, 8))
              }
              inputMode="numeric"
              maxLength={8}
              autoComplete="off"
      ...
      <SettingsSection title={tr("remote.access")} target="remote-access">
        ...
        <SettingsRow>
          ...
          <Input
            ...
            onChange={(event: any) => setName((event.target as HTMLInputElement).value)}
          />
        </SettingsRow>
        <SettingsRow>
          <SettingsCopy>

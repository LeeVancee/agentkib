...
export function AssetCatalogPage({ assets, workspaces, onOpen }: AssetCatalogPageProps) {
  ...
  const [page, setPage] = useLinkedState<
    {
      agent: "all" | AgentKind;
      kind: string;
      ownership: "all" | "shared" | "native";
      query: string;
      workspaceId: string;
    },
    number
  >(
    ...
    (next, previous) =>
      next.agent === previous?.source.agent &&
      next.kind === previous?.source.kind &&
      next.ownership === previous?.source.ownership &&
      next.query === previous?.source.query &&
      next.workspaceId === previous?.source.workspaceId
        ? previous.value
        : 1,
    {

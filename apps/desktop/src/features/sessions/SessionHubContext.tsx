...
function useHub(active: boolean) {
  ...
  useLayoutEffect(() => {
    const completed = wasRefreshing.current && !catalog.refreshing && catalog.ready && enabled;
    wasRefreshing.current = catalog.refreshing;
    if (completed)
      void Promise.resolve().then(() => setHistoryRevision((revision) => revision + 1));
  ...
  const select = (id?: string, replace = false) =>
    void navigate({
      to: "/sessions",
      replace,
      search: (current: any) => ({ ...current, sessionId: id }),
    });
  ...
  useEffect(() => {
    ...
    if (
      ...
      void navigate({
        to: "/sessions",
        replace: true,
        search: (current: any) => ({ ...current, sessionId: undefined }),
      });
    }
  }, [
    sessionId,
